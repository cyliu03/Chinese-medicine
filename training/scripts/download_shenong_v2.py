"""
使用huggingface_hub下载ShenNong TCM数据集
"""
...
def download_shenong_dataset():
    ...
    """
    print("=" * 70)
    print("  下载ShenNong TCM数据集 (11万+中医药数据)")
    print("=" * 70)
    ...
    try:
        print("\n正在从HuggingFace下载数据集...")
        print("数据集: michaelwzhu/ShenNong_TCM_Dataset")
        
        # 下载数据集
        local_dir = os.path.join(DATASET_DIR, "shenong")
...
def parse_shenong_data(data_dir):
    """
    解析ShenNong数据集
    """
    print("\n解析ShenNong数据集...")
...
def extract_symptoms(data):
    ...
    input_text = data.get("input", "") + " " + data.get("instruction", "")
    ...
    symptom_keywords = [
        ...
        "恶风", "汗出", "寒热往来", "胸胁苦满", "烦躁", "口渴",
        "四肢厥冷", "神疲", "畏寒", "下利清谷", "半身不遂",
        "口眼歪斜", "语言謇涩", "小便频数", "肢体酸痛", "肢体困重",
        "脘腹胀满", "嗳腐吞酸", "情绪抑郁", "咽中如有物阻",
        "呼多吸少", "哮喘", "嗳气", "呃逆", "胃脘痞硬",
        "斑疹", "出血", "谵语", "神昏", "牙关紧闭", "突然昏倒",
    ]
...
def extract_herbs(data):
    ...
    herb_keywords = [
        ...
        "山药", "山茱萸", "泽泻", "附子", "干姜", "肉桂",
        "吴茱萸", "小茴香", "丁香", "高良姜", "胡椒", "花椒",
        "荜茇", "荜澄茄", "细辛", "白芷", "辛夷", "苍耳子",
        "鹅不食草", "西河柳", "胡荽", "香薷", "藿香", "佩兰",
        "苍术", "砂仁", "白豆蔻", "草豆蔻", "草果", "厚朴花",
        "扁豆", "扁豆花", "薏苡仁", "芡实", "莲子", "莲须",
        "荷叶", "荷梗", "藕节", "木通", "通草", "灯心草",
        "瞿麦", "萹蓄", "地肤子", "海金沙", "石韦", "冬葵子",
        "车前子", "滑石", "茵陈", "金钱草", "虎杖", "垂盆草",
        "地耳草", "鸡骨草", "珍珠草", "附子", "干姜", "肉桂",
        "吴茱萸", "小茴香", "丁香", "高良姜", "胡椒", "花椒",
    ]
...
def merge_and_split_data(shenong_samples, classic_samples):
    """合并并划分数据集"""
    print("\n合并数据集...")
...
def main():
    ...
    # 下载ShenNong数据集
    data_dir = download_shenong_dataset()
    
    # 解析ShenNong数据
    shenong_samples = []
    if data_dir:
        shenong_samples = parse_shenong_data(data_dir)
